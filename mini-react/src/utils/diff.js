import handleAttrs from '../react-dom/handleAttrs';
...
export function diffNode(dom, vnode) {
  ...
  // 对比非文本节点
  // 1. 真实DOM不存在，表示此节点是新增的
  // 2. 新旧两个节点的类型不一样，那么就新建一个DOM元素，并将原来的子节点（如果有的话）移动到新建的DOM节点下
...
function diffChildren(dom, vchildren) {
  ...
  const children = [];  // 没有key值的真实dom集合
  const keyed = {};     // 有key值的集合
